...
export function useSafePageViewTracker(options: TrackingOptions = {}) {
  ...
  const trackPageViewSafely = useCallback(
    async (data: TrackingData, useBeacon: boolean = false): Promise<void> => {
  ...
  const startIntervalTracking = useCallback(
    (trackingData: Omit<TrackingData, "duration">) => {
