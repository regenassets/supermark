...
async function fetchAndCacheDurations(
  ...
  if (cachedDurations) {
    const parsedDurations =
      typeof cachedDurations === "string"
        ? JSON.parse(cachedDurations)
        : cachedDurations;
...
export default async function handle(
  ...
  if (req.method === "GET") {
    ...
    const { teamId, id, page, pageSize, sortBy, sortOrder, withDuration } =
      req.query as {
        teamId: string;
        id: string;
        page?: string;
    ...
    try {
      ...
      if (sort === "totalDuration") {
        ...
        const durationsMap = await fetchAndCacheDurations(
          allDocuments,
          teamId,
          id,
          durationCacheKey,
